...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # The truncated length should be at least 4 and at most int(self.block_size*3/4).
        max_trunc_length = int(self.block_size * 3 / 4)
        # Ensure we do not exceed the document length.
        trunc_upper_bound = min(max_trunc_length, len(doc))
        if trunc_upper_bound < 4:
            truncated_doc = doc  # Fall-back if doc is very short.
        else:
            trunc_length = random.randint(4, trunc_upper_bound)
            truncated_doc = doc[:trunc_length]
        
        L_trunc = len(truncated_doc)
        
        # 2. Break the truncated document into three parts: [prefix], [masked_content], [suffix].
        # Choose masked_content length randomly.
        # We pick masked_length from 1 up to L_trunc//2; note that the average of such a uniform sample is roughly L_trunc/4.
        if L_trunc >= 2:
            masked_length = random.randint(1, max(1, L_trunc // 2))
        else:
            masked_length = 1
        
        # Now choose the splitting point for prefix (and so the remainder splits into masked_content and suffix).
        prefix_length = random.randint(0, L_trunc - masked_length)
        ...
        # 3. Rearrange these substrings into the specified form:
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] MASK_CHAR [pads]
        masked_string_no_pad = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR
        # Determine number of PAD_CHAR's required to reach self.block_size.
        pad_length = self.block_size - len(masked_string_no_pad)
        if pad_length > 0:
            pads = self.PAD_CHAR * pad_length
            masked_string = masked_string_no_pad + pads
        else:
            # If the constructed string is too long, truncate it.
            masked_string = masked_string_no_pad[:self.block_size]
        
        # 4. Construct input and output examples: input is masked_string[:-1], output is masked_string[1:].
        x_str = masked_string[:-1]
        y_str = masked_string[1:]
        
        # 5. Encode the strings using the vocabulary (self.stoi) into Long tensors.
        x_tensor = torch.tensor([self.stoi[c] for c in x_str], dtype=torch.long)
        y_tensor = torch.tensor([self.stoi[c] for c in y_str], dtype=torch.long)
